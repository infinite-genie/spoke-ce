...
- **Color tokens → inline custom properties** named `--spk-bg`, `--spk-fg`, `--spk-bg-hover`, `--spk-border`, `--spk-presence`. Build the style object then cast: `const style = { '--spk-bg': cssVar('primary'), minHeight: space[8] } as CSSProperties;`.
